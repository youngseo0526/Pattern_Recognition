...
res[conf_map > threshold] = [255, 0, 0]   # C가 일정 임계값보다 크면 corner (B)
res[conf_map <= threshold] = [0, 0, 255]  # C가 0에 가까우면(small) flat (R)
res[conf_map < 0] = [255, 255, 255]       # C가 음수이면 edge (W)

# 결과 시각화
concatenated = np.hstack((img, res))  # horizontal stack - img, res 2개의 이미지를 수평 방향의 하나의 이미지로 묶음
concatenated = cv2.resize(concatenated, None, fx=2.0, fy=2.0)  # 이미지 확대하기 위함. 중요하지 않음
cv2.imshow('harris corner', concatenated)
cv2.waitKey(0)
cv2.imwrite('result shapes.png', concatenated)
